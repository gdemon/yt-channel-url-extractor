...
- `run_pipeline.py`：最高階的一鍵執行流水線，自動整合 `main.py` 與 `asr_converter.py` 處理下載並轉譯流程（若預期下載之音檔已存在，則自動跳過下載與轉譯以節省系統資源）。
- `run_pipeline_api.py`：與 `run_pipeline.py` 類似，但改採 `scripts/transcribe_api.py`（Google Speech Recognition API）進行音檔轉譯，不需本機 GPU 與下載 Whisper 模型。
